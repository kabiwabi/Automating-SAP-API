...
public class AllWeatherTest extends TestUtilities {
    ...
    private String allWeatherTag = "All-Weather";

    @Test
    @Parameters({"hybrisUsername", "hybrisPassword", "inputTireSize"})
    public void allWeatherTest(String hybrisUsername, String hybrisPassword, String inputTireSize) throws IOException {
        ...
        for(int i=0; i<tireListFull.size(); i++) {
            ...
            Assert.assertEquals(tireProductIterate.getSeasonTag(), allWeatherTag, "Season verification failed!");
            log.info("Successfully verified that product "+i+"'s season tag matches the expected tag of "+allWeatherTag);
        }
    }
}
